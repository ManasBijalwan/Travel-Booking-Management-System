...
const defaultStop = { location_id: "", arrival_time: "", departure_time: "" };
...
function AdminRoutesPage() {
  ...
  const removeStop = (index) => {
    setForm((current) => ({
      ...current,
      intermediateStops: current.intermediateStops.filter((_, i) => i !== index),
    }));
  };
  ...
  const handleSubmit = async (event) => {
    ...
    const diffDays = Math.floor(
      (new Date(form.bookingEndDate) - new Date(form.bookingStartDate)) /
        (1000 * 60 * 60 * 24)
    );
    ...
    <section className="page-shell">
      <div className="section-heading">
        <div>
          ...
          <h1>Routes</h1>
          <p>
            Configure route details, active days, booking window, linked
            operator and vehicle, and stop timings.
          </p>
        </div>
      </div>
              ...
              <tbody>
                {filteredRecords.map((record) => (
                  <tr key={record.id}>
                    ...
                    <td>{record.origin_name} → {record.destination_name}</td>
                    ...
                    <td>
                      {record.intermediateStops?.length
                        ? record.intermediateStops
                            .map((s) => `${s.location_name} (${s.arrival_time}–${s.departure_time})`)
                            .join(", ")
                        : "Direct"}
                    </td>
                    <td>
