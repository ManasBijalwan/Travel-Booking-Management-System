...
const { execute, executeWithConn, fetchCursor } = require("../db");
...
function mapScheduleRow(row) {
  return {
    ...
    seatMap: buildSeatMap(row.TOTAL_SEATS || 0),
    status: row.STATUS,
  };
}

// ─── GET /api/travels ─────────────────────────────────────────────────────────
// Query params: origin (city/name), destination, departureDate, type (mode name)
router.get("/", authenticate, requireUser, async (req, res) => {
  try {
    ...
    if (origin) {
      conditions.push(
        "(UPPER(l_start.city) LIKE UPPER(:origin) OR UPPER(l_start.location_name) LIKE UPPER(:origin))"
      );
      binds.origin = `%${origin}%`;
    }
    if (destination) {
      conditions.push(
        "(UPPER(l_end.city) LIKE UPPER(:dest) OR UPPER(l_end.location_name) LIKE UPPER(:dest))"
      );
    ...
    const sql = `
      SELECT s.schedule_id,
             ...
             tm.mode_name,
             op.operator_name,
             l_start.location_name  AS start_location,
             l_start.city           AS start_city,
             l_end.location_name    AS end_location,
             l_end.city             AS end_city,
             (
        ...
        JOIN LOCATION l_start ON r.start_location_id = l_start.location_id
        JOIN LOCATION l_end   ON r.end_location_id   = l_end.location_id
